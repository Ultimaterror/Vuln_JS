		...
		<div className="p-4 flex mt-7">
			<div className="man-bg container mx-auto px-4">
				<h1 className="text-4xl font-bold mb-8">
					Administration du Blog (Réservé aux administrateurs)
				</h1>
				...
				<section className="mb-12">
					...
					<ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{articles && articles.length > 0 ? (
							articles.map((article) => (
								<li key={article.id} className="border p-4 rounded shadow-md">
									<div className="flex justify-between mb-2">
										...
										<p className="text-info">
											Par{" "}
											{users.find((u) => u.id === article.author_id)?.username}
										</p>
									</div>
									...
									<ul className="space-y-2">
										{article.comments &&
											article.comments.map((comment) => (
												<li
													key={comment.id}
													className="border p-2 rounded bg-gray-100 flex justify-between"
												>
													<div>
														<p className="text-info">
															Par{" "}
															{
																users.find(
																	(u) =>
																		Number(u.id) === Number(comment.user_id)
																)?.username
															}
														</p>
														<p>{comment.content}</p>
													...
													<button
														onClick={() =>
															handleDeleteComment(comment.id, article.id)
														}
